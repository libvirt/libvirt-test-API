...
from libvirttestapi.repos.domain.domain_common import get_flags, get_fileflags
from libvirttestapi.repos.domain.domain_common import check_fileflag, check_dom_state
...
def save_flags(params):
    ...
    if flags == -1:
        return 1
    if flags == 0:
        logger.info("save %s domain with no flag" % guestname)
    elif flags == 1:
        logger.info("save %s domain with --bypass-cache" % guestname)
    elif flags == 2:
        logger.info("save %s domain with --running" % guestname)
    elif flags == 3:
        logger.info("save %s domain with --bypass-cache --running" % guestname)
    elif flags == 4:
        logger.info("save %s domain with --paused" % guestname)
    elif flags == 5:
        logger.info("save %s domain with --bypass-cache --paused" % guestname)
    ...
    if flags % 2 == 1:
        ...
        # For save, get file flag from /proc/PID/fdinfo/1
        thread.start_new_thread(get_fileflags, (save_path, fileflags, "1", logger,))

    # Guarantee get_fileflags shell has run before save
    time.sleep(5)
